...
class TSVCriteoIterDataPipe(IterableDataset):
    """
    Datapipe designed to operate over binary (npy) versions of Criteo datasets. Loads
    the entire dataset into memory to prevent disk speed from affecting throughout. Each
    rank reads only the data for the portion of the dataset it is responsible for.
    ...
    def __init__(
        ...
        self.datapipe = criteo_terabyte(tsv_paths)
        self.datapipe = dp.iter.Batcher(self.datapipe, batch_size)
        self.datapipe = dp.iter.Collator(self.datapipe)
    ...
    def _iter_to_batch(
        self, batch # dense: np.ndarray, sparse: np.ndarray, labels: np.ndarray
    ) -> Batch:
        # if self.shuffle_batches:
        #     # Shuffle all 3 in unison
        #     shuffler = np.random.permutation(len(dense))
    ...
    def __iter__(self) -> Iterator[Batch]:
        ...
        yield self._iter_to_batch(batch)
